...
(function(){
  ...
  function syncWorker(list){if(!('serviceWorker' in navigator))return;navigator.serviceWorker.ready.then(reg=>{if(reg.active)reg.active.postMessage({type:'SET_REMINDERS',items:list.map(x=>({id:x.key,nome:x.message,data:x.date,paga:false}))})}).catch(()=>{});}
  ...
  function run(){const list=collect();panel(list);syncWorker(list);notify(list)}
