...
(function(){
  ...
  function collectAll(){const db=read(),out=[];const add=(arr,nome)=> (arr||[]).forEach(x=>{if(!x.paga&&x.data)out.push({id:nome+'-'+(x.id||x.data),nome:nome,data:x.data,paga:false})});add(db.ipva,'IPVA');add(db.seguro,'Seguro');add(db.multas,'Multa');(db.manutencoes||[]).forEach(x=>{if(x.nextDate)out.push({id:'manut-date-'+x.id,nome:'Manutenção — '+(x.servico||'serviço'),data:x.nextDate,paga:false})});return out;}
