...
function calc(){
 ...
 const initial=brNumber(document.getElementById('kmc-initial').value), final=brNumber(document.getElementById('kmc-final').value);
 const km=Math.max(0,final-initial);
 let base=service.startsWith('km:') ? km*Number(service.slice(3)) : Number(service||0);
 const extraHours=brNumber(document.getElementById('kmc-hours').value)*90;
 const excess=isKm()?0:brNumber(document.getElementById('kmc-excess').value)*3;
...
function render(){
 ...
 ['kmc-initial','kmc-final','kmc-hours','kmc-excess'].forEach(id=>document.getElementById(id).addEventListener('input',calc));
