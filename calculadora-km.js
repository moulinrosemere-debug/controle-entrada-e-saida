...
function formatDuration(sec){sec=Math.max(0,Math.floor(sec||0));const h=Math.floor(sec/3600),m=Math.floor((sec%3600)/60),s=sec%60;return [h,m,s].map((v,i)=>i===0?String(v).padStart(2,'0'):String(v).padStart(2,'0')).join(':');}
...
function buildMessage(total,km,rate,kmValue,serviceBase,hours,excess,toll,parking,waiting){
 ...
 return `*LOPES TUR — COTAÇÃO*\n\nCliente: ${client}${isManual?'':`\nServiço: ${label}`}\nKM rodados: ${numberKm(km)}${rate?'\nValor por KM: '+money(rate):''}${kmValue?'\nValor dos KM: '+money(kmValue):''}${serviceBase?'\nValor do serviço: '+money(serviceBase):''}${hours?'\nHoras extras: '+money(hours):''}${excess?'\nQuilometragem excedente: '+money(excess):''}${waitSec?'\nTempo de espera: '+formatDuration(waitSec)+' — '+money(waiting):''}${toll?'\nPedágio: '+money(toll):''}${parking?'\nEstacionamento: '+money(parking):''}\n\n*TOTAL: ${money(total)}*\n\nLopes Tur — Transporte Executivo`;
}
function startWait(){if(waitStartedAt)return;waitStartedAt=Date.now();waitElapsedSeconds=0;clearInterval(waitTimer);waitTimer=setInterval(()=>{waitElapsedSeconds=Math.floor((Date.now()-waitStartedAt)/1000);updateWaitDisplay();calc();},1000);document.getElementById('kmc-wait-start').disabled=true;document.getElementById('kmc-wait-stop').disabled=false;updateWaitDisplay();}
function stopWait(){if(!waitStartedAt)return;waitElapsedSeconds=Math.floor((Date.now()-waitStartedAt)/1000);clearInterval(waitTimer);waitTimer=null;waitStartedAt=null;document.getElementById('kmc-wait-start').disabled=false;document.getElementById('kmc-wait-stop').disabled=true;updateWaitDisplay();calc();}
function resetWait(){clearInterval(waitTimer);waitTimer=null;waitStartedAt=null;waitElapsedSeconds=0;const a=document.getElementById('kmc-wait-start'),b=document.getElementById('kmc-wait-stop');if(a)a.disabled=false;if(b)b.disabled=true;updateWaitDisplay();calc();}
