(function(){
'use strict';
const CALC_KEY='lopesTur.calculadoraKM.v1';
...
function readSaved(){try{return JSON.parse(localStorage.getItem(CALC_KEY))||{}}catch(e){return {}}}
function saveSaved(data){try{localStorage.setItem(CALC_KEY,JSON.stringify(data));window.dispatchEvent(new CustomEvent('lopes-tur:calculadora-saved'));}catch(e){}}
let waitTimer=null;
...
function calc(){
 ...
 const odometerStart=brNumber(document.getElementById('kmc-odometer-start')?.value);
 const odometerEnd=brNumber(document.getElementById('kmc-odometer-end')?.value);
 const odometerKm=(odometerEnd>=odometerStart&&odometerEnd>0)?odometerEnd-odometerStart:0;
 const manualKmEl=document.getElementById('kmc-manual-km');
 if(manualKmEl&&odometerStart>0&&odometerEnd>=odometerStart){manualKmEl.value=String(odometerKm);manualKmEl.readOnly=true;}
 const km=Math.max(0,brNumber(manualKmEl?.value));
...
function buildMessage(total,km,rate,kmValue,serviceBase,hours,excess,toll,parking,waiting){
 ...
 return `*LOPES TUR — COTAÇÃO*\n\nCliente: ${client}${isManual?'':`\nServiço: ${label}`}\nOdômetro: ${numberKm(brNumber(document.getElementById('kmc-odometer-start')?.value))} → ${numberKm(brNumber(document.getElementById('kmc-odometer-end')?.value))}\nKM rodados: ${numberKm(km)}${rate?'\nValor por KM: '+money(rate):''}${kmValue?'\nValor dos KM: '+money(kmValue):''}${serviceBase?'\nValor do serviço: '+money(serviceBase):''}${hours?'\nHoras extras: '+money(hours):''}${excess?'\nQuilometragem excedente: '+money(excess):''}${waitSec?waitTime+'\nTempo de espera: '+formatDuration(waitSec)+' — '+money(waiting):''}${toll?'\nPedágio: '+money(toll):''}${parking?'\nEstacionamento: '+money(parking):''}\n\n*TOTAL: ${money(total)}*\n\nLopes Tur — Transporte Executivo`;
}
function currentData(){
 const ids=['kmc-odometer-start','kmc-odometer-end','kmc-client','kmc-service','kmc-manual-km','kmc-km-rate','kmc-hours','kmc-excess','kmc-wait-rate','kmc-wait-start-time','kmc-wait-end-time','kmc-toll','kmc-parking'];
 const data={};ids.forEach(id=>{const el=document.getElementById(id);if(el)data[id]=el.value;});
 data.waitElapsedSeconds=waitElapsedSeconds;data.manualStartTime=manualStartTime;data.manualEndTime=manualEndTime;data.savedAt=new Date().toISOString();return data;
}
function saveCurrent(){saveSaved(currentData());}
function restoreSaved(){const d=readSaved();if(!d||!Object.keys(d).length)return;Object.keys(d).forEach(id=>{const el=document.getElementById(id);if(el&&id.startsWith('kmc-'))el.value=d[id]??'';});manualStartTime=d.manualStartTime||document.getElementById('kmc-wait-start-time')?.value||'';manualEndTime=d.manualEndTime||document.getElementById('kmc-wait-end-time')?.value||'';waitElapsedSeconds=Number(d.waitElapsedSeconds||0);const a=document.getElementById('kmc-wait-start-time'),b=document.getElementById('kmc-wait-end-time');if(a)a.value=manualStartTime;if(b)b.value=manualEndTime;}
function startWait(){if(waitStartedAt)return;manualStartTime='';manualEndTime='';const now=new Date();manualStartTime=now.toTimeString().slice(0,5);waitStartedAt=Date.now();waitElapsedSeconds=0;clearInterval(waitTimer);waitTimer=setInterval(()=>{waitElapsedSeconds=Math.floor((Date.now()-waitStartedAt)/1000);updateWaitDisplay();calc();},1000);document.getElementById('kmc-wait-start').disabled=true;document.getElementById('kmc-wait-stop').disabled=false;const st=document.getElementById('kmc-wait-start-time');if(st)st.value=manualStartTime;updateWaitDisplay();saveCurrent();}
function stopWait(){if(!waitStartedAt)return;waitElapsedSeconds=Math.floor((Date.now()-waitStartedAt)/1000);const now=new Date();manualEndTime=now.toTimeString().slice(0,5);clearInterval(waitTimer);waitTimer=null;waitStartedAt=null;document.getElementById('kmc-wait-start').disabled=false;document.getElementById('kmc-wait-stop').disabled=true;const en=document.getElementById('kmc-wait-end-time');if(en)en.value=manualEndTime;updateWaitDisplay();calc();}
...
function render(){
 ...
 document.getElementById('kmc-calc').onclick=calc;document.getElementById('kmc-whatsapp').onclick=share;document.getElementById('kmc-clear').onclick=()=>{localStorage.removeItem(CALC_KEY);render();};document.getElementById('kmc-wait-start').onclick=startWait;document.getElementById('kmc-wait-stop').onclick=stopWait;['kmc-odometer-start','kmc-odometer-end','kmc-client','kmc-manual-km','kmc-km-rate','kmc-hours','kmc-excess','kmc-wait-rate','kmc-toll','kmc-parking'].forEach(id=>document.getElementById(id).addEventListener('input',calc));document.getElementById('kmc-service').addEventListener('change',calc);moneyInput('kmc-km-rate');moneyInput('kmc-toll');moneyInput('kmc-parking');calc();
}
